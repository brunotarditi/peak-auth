...
type UserService interface {
	...
	Login(req requests.LoginRequest, publicAppID string) (responses.TokenResponse, error)
...
func NewUserService(userRepo repositories.UserRepository, roleRepo repositories.RoleRepository, uarRepo repositories.UserApplicationRoleRepository, appRepo repositories.ApplicationRepository, ruleService ApplicationRuleService, tokenManager *auth.JWTManager, emailVerificationRepo repositories.EmailVerificationRepository, passwordResetRepo repositories.PasswordResetRepository) UserService {
...
func (s *userService) Login(req requests.LoginRequest, publicAppID string) (responses.TokenResponse, error) {
	// 1. Validar Identidad
	user, err := s.userRepo.FindByEmail(req.Email)
	if err != nil || !utils.CheckPasswordHash(req.Password, user.Password) {
		return responses.TokenResponse{}, fmt.Errorf("credenciales inválidas")
	}
	...
	if err != nil {
		return responses.TokenResponse{}, fmt.Errorf("aplicación no autorizada")
	}
	...
	return responses.TokenResponse{AccessToken: token}, nil
}
...
func (s *userService) Register(req requests.RegisterRequest) (models.User, error) {
	...
	if err != nil {
		return models.User{}, fmt.Errorf("aplicación no encontrada")
	}
	...
	if err == nil {
		...
		if roles, rerr := s.uarRepo.FindRolesByUserAndApp(user.ID, app.ID); rerr == nil && len(roles) > 0 {
			return models.User{}, fmt.Errorf("el email ya está registrado en esta aplicación")
		}
	} else if !errors.Is(err, gorm.ErrRecordNotFound) {
		return models.User{}, fmt.Errorf("error verificando usuario: %w", err)
	}
	...
	if !userExists {
		nu, _ := req.ToUser()
		profile := models.Profile{FirstName: req.FirstName, LastName: req.LastName}
...
func (s *userService) AdminLogin(email, password string) (string, error) {
	...
	// generar JWT admin con expiración 12h
	token, err := s.tokenManager.GenerateToken(user.ID, user.Email, "PEAK_AUTH", 12*time.Hour)
...
func (s *userService) FindUserByAppID(appID string) ([]models.User, error) {
	app, err := s.appRepo.FindByAppID(appID)
	if err != nil {
		return nil, fmt.Errorf("aplicación no encontrada")
	}

	users, err := s.uarRepo.GetUsersByApp(app.ID)
	if err != nil {
