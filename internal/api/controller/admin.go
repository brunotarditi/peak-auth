...
func (ctrl *AdminController) PostResendVerification(c *gin.Context) {
	appID := c.Param("id")
	userIDStr := c.Param("user_id")
	var userID uint
	fmt.Sscanf(userIDStr, "%d", &userID)
...
func (ctrl *AdminController) PostSendResetPassword(c *gin.Context) {
	userIDStr := c.Param("user_id")
	var userID uint
	fmt.Sscanf(userIDStr, "%d", &userID)
	...
	if err != nil {
		c.JSON(http.StatusInternalServerError, gin.H{"error": "Error verificando límites"})
		return
	}
